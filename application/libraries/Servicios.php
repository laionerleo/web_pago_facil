<?php if ( ! defined('BASEPATH')) exit('No se permite el acceso directo al script'); class servicios {
   ...
   public function registro_parcial($nombre,$apellido,$correo)
   {
    ...
    $url = 'http://serviciopagofacil.syscoop.com.bo/api/Usuario/registrarByEmail';
     $data = array('tcNombre' => $nombre, 'tcApellido' => $apellido,'tcDireccion' => '' ,'tcCinit' => '' , 'tcFacturaa' => 0,'tnTelefono' =>0 ,'tnTelefonoPago' =>0 , 'tcCorreo' =>$correo ,'tcLogin' =>$correo ,'tcPassword' => 
$contraseña ,'tcImei' => $imei , 'tcApp'=>2 );
   ...
   public function registropagofacil($nombre,$apellido,$correo,$numero,$login,$contraseña)
   {
     // $contraseña="25d55ad283aa400af464c76d713c07ad";
      $imei=9999;
    $url = 'http://serviciopagofacil.syscoop.com.bo/api/Usuario/registrarByEmail';
     $data = array('tcNombre' => $nombre, 'tcApellido' => $apellido,'tcDireccion' => '' ,'tcCinit' => '' , 'tcFacturaa' => 0,'tnTelefono' =>$numero ,'tnTelefonoPago' =>0 , 'tcCorreo' =>$correo ,'tcLogin' =>$login ,'tcPassword' => 
$contraseña ,'tcImei' => $imei , 'tcApp'=>2 );
   ...
   public function bcpconfirmarpago($tnCliente,$tnEmpresa , $tnAuthorizationNumber,$tnCorrelationId , $tcOTP )
   {
    ...
    Call<mPaquetePagoFacil<String>> BCP_ConfirmarPago(
            ...
            [13:00, 9/6/2020] Huga Syscoop: y ese OTP se ingresa al metodo confirmar Pago [13:01, 9/6/2020] Huga Syscoop: El preprarar pago devuleve elpaquete pagoFacil de siempre, en values devuelves una cadena separada asi: 
[13:01, 9/6/2020] Huga Syscoop: 5555;666 [13:01, 9/6/2020] Huga Syscoop: donde 555 es el numero de transaccion que se creo y 666 el codigo de autorizacion de BCP [13:02, 9/6/2020] Huga Syscoop: ambos datos los usara en elmetodo 
confirmarPAgo [13:02, 9/6/2020] Huga Syscoop: tnCorrelationId= trasnaccionDePago tnAuthorizationNumber= autorizacion deBCP */
   ...
   public function ejecuparpagoelinkser($tncliente,$tnempresa,$codigoclienteempresa, $tnmetodopago,$tnTelefono , $tcFacturaA , $tnCiNit ,$tcNroPago ,$tnMontoClienteEmpresa , $tnMontoClienteSyscoop , $tcPeriodo , $tcImei , 
$tcTarjeta , $tcTarjetaHabiente , $tcCodigoSeguridad , $tcFechaExpiracion ,  $taFacturas , $taDatosSintesis   )
   {
      $url = 'http://serviciopagofacil.syscoop.com.bo/api/Factura/Linkser_RealizarPago';
      $data = array('tnCliente' =>$tncliente , 'tnEmpresa' => $tnempresa , 'tcCodigoClienteEmpresa' => (String) $codigoclienteempresa ,'tnMetodoPago' => $tnmetodopago ,'tnTelefono' => (String)$tnTelefono ,'tcFacturaA' => (String) 
$tcFacturaA ,'tnCiNit' =>(String) $tnCiNit ,'tcNroPago' =>(String) $tcNroPago, 'tnMontoClienteEmpresa' => (string)($tnMontoClienteEmpresa) , 'tnMontoClienteSyscoop' =>(string)$tnMontoClienteSyscoop , 'tcPeriodo' =>(String) 
$tcPeriodo , 'tcImei'=> (String)$tcImei , 'tcTarjeta' =>(String) $tcTarjeta , 'tcTarjetaHabiente' =>$tcTarjetaHabiente, 'tcCodigoSeguridad' => (String)$tcCodigoSeguridad, 'tcFechaExpiracion' => (String)$tcFechaExpiracion , 
'tcApp'=>2 ,"taDetallePago"=>$taFacturas , "taDatosSintesis"=>$taDatosSintesis);
   ...
   public function actualizardatospagofacil($tnCliente , $tcNombre , $tcApellido , $tcDireccion , $tcCinit , $tcFacturaA , $tnTelefono , $tnTelefonoDePago)
    {
      ...
      //tnCliente, tnTransaccion, tcApp
      $data = array( 'tnCliente' =>$tnCliente ,'tcNombre'=> $tcNombre , 'tcApellido' => $tcApellido ,'tcDireccion' => $tcDireccion ,'tcCinit' => $tcCinit ,'tcFacturaA' => $tcFacturaA ,'tnTelefono' => strval($tnTelefono) 
,'tnTelefonoDePago' => strval($tnTelefonoDePago ) , 'tcApp'=>2 );
...
//$taEntidades, $taFacturas , $taDatosSintesis )
public function generarqr($laDataGenerarqr )
    ...
    public function generarqratc($laDataGenerarqr )
    {
 
       $url = 'http://serviciopagofacil.syscoop.com.bo/api/Factura/ATC_GenerarQR';
       $data = $laDataGenerarqr ; //array('tnCliente' => strval($tnCliente ) , 'tnEmpresa' => $tnEmpresa , 'tcCodigoClienteEmpresa' => $tcCodigoClienteEmpresa , 'tnMetodoPago'=> $tnMetodoPago , 'tnTelefono'=> $tnTelefono , 'tcFacturaA'=> $tcFacturaA , 'tnCiNit'=> $tnCiNit ,'tcNroPago'=> $tcNroPago ,'tnMontoClienteEmpresa' => $tnMontoClienteEmpresa ,'tnMontoClienteSyscoop' => $tnMontoClienteSyscoop ,'tcPeriodo'=>$tcPeriodo ,'tcImei'=> $tcImei ,'tcApp'=>2 , 'taEntidades'=> $taEntidades , "taDetallePago"=>$taFacturas , "taDatosSintesis"=>$taDatosSintesis );
       $this->cargarlog("generarqr".json_encode($data));
    ...
    public function generarqrpix($tnCliente , $tnEmpresa ,$tcCodigoClienteEmpresa ,$tnMetodoPago , $tnTelefono ,$tcFacturaA , $tnCiNit ,$tcNroPago , $tnMontoClienteEmpresa , $tnMontoClienteSyscoop ,$tcPeriodo ,$tcImei , 
$taEntidades , $taFacturas , $taDatosSintesis )
    {
 
      $url = 'http://serviciopagofacil.syscoop.com.bo/api/Factura/PIX_GenerarQR';
      $data = array('tnCliente' => strval($tnCliente ) , 'tnEmpresa' => $tnEmpresa , 'tcCodigoClienteEmpresa' => $tcCodigoClienteEmpresa , 'tnMetodoPago'=> $tnMetodoPago , 'tnTelefono'=> $tnTelefono , 'tcFacturaA'=> $tcFacturaA , 
'tnCiNit'=> $tnCiNit ,'tcNroPago'=> $tcNroPago ,'tnMontoClienteEmpresa' => $tnMontoClienteEmpresa ,'tnMontoClienteSyscoop' => $tnMontoClienteSyscoop ,'tcPeriodo'=>$tcPeriodo ,'tcImei'=> $tcImei ,'tcApp'=>2 , 'taEntidades'=> 
$taEntidades , "taDetallePago"=>$taFacturas , "taDatosSintesis"=>$taDatosSintesis, "tnMoneda"=>2 );
    ...
    public function generarpagobinance($tnCliente , $tnEmpresa ,$tcCodigoClienteEmpresa ,$tnMetodoPago , $tnTelefono ,$tcFacturaA , $tnCiNit ,$tcNroPago , $tnMontoClienteEmpresa , $tnMontoClienteSyscoop ,$tcPeriodo ,$tcImei , 
$taEntidades , $taFacturas , $taDatosSintesis )
    {
 
      $url = 'https://serviciobinance.pagofacil.com.bo/api/servicio/generarpagobinance';
      $data = array('tnCliente' => strval($tnCliente ) , 'tnEmpresa' => $tnEmpresa , 'tcCodigoClienteEmpresa' => $tcCodigoClienteEmpresa , 'tnMetodoPago'=> $tnMetodoPago , 'tnTelefono'=> $tnTelefono , 'tcFacturaA'=> $tcFacturaA , 
'tnCiNit'=> $tnCiNit ,'tcNroPago'=> $tcNroPago ,'tnMontoClienteEmpresa' => $tnMontoClienteEmpresa ,'tnMontoClienteSyscoop' => $tnMontoClienteSyscoop ,'tcPeriodo'=>$tcPeriodo ,'tcImei'=> $tcImei ,'tcApp'=>2 , 'taEntidades'=> 
$taEntidades , "taDetallePago"=>$taFacturas , "taDatosSintesis"=>$taDatosSintesis, "tnMoneda"=>2 );
    ...
    public function genentidadesfinancieras($tnCliente ,$tnMetodoPago = 4 )
    {
       $url = 'http://serviciopagofacil.syscoop.com.bo/api/Factura/getEntidadesFinancieras';
       $data = array('tnCliente' => strval($tnCliente ) , 'tcApp'=>2 , 'tnMetodoPago'=>$tnMetodoPago );
       /* echo "<pre>";
       print_r(json_encode($data));
       echo "</pre>";
       */
    ...
    public function getcodigosservicio($tnCliente, $tnEmpresa)
    {
      ...
      /* [13:21, 18/8/2020] Victorhugo Syscoop: @Field("tnCliente") long cliente,
            @Field("tnEmpresa") long empresa,
            @Field("tcCodigoClienteEmpresa") String tcCodigoClienteEmpresa); [13:21, 18/8/2020] Victorhugo Syscoop: recibe esos 3 parametros [13:21, 18/8/2020] Victorhugo Syscoop: el tcCodigoClienteEmpresa envia "0"
      */
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
 ...
 /*
  tnCliente tcEntidades tnTransaccion
   */
   $header = array(
      "Content-Type: application/x-www-form-urlencoded",
      "Content-Length: ".strlen( http_build_query($data))
...
public function getPuntoCobranza($tnCliente){
   ...
   /* @POST(cPagoFacilPHP + "/Usuario/getPuntoDeCobranza") @FormUrlEncoded Call<mPaquetePagoFacil<ArrayList<PuntoCobranza>>> getPuntoCobranza(
        @Field("tnCliente") long tnCliente,
        @Field("tnIdAccion") int tnIdAccion);;
   */
   $header = array(
...
public function jwtvalidation($tcJWT , $tnMetodoPago , $taDataCardinal) {
   ...
   /* //https://serviciopagofacil.syscoop.com.bo/api/jwt_validation tcJWT
    */
   $header = array(
      "Content-Type: application/x-www-form-urlencoded",
      "Content-Length: ".strlen( http_build_query($data))
...
public function cybersourseinit($tcApiRest, $tnCantidad , $tnMetodoPago) {
 ...
 /* [9:00, 5/1/2021] Huga Syscoop: tnCantidad [9:01, 5/1/2021] Huga Syscoop: tcApiRest */
...
public function getestados($tncliente,$tnPais) {
   ...
   /*
   @POST(cPagoFacilPHP + "/Factura/calcularComision") @FormUrlEncoded Call<mPaquetePagoFacil<Double>> calcularComision
         (@Field("tnEmpresa") long empresa,
         @Field("tnCliente") long cliente,
         @Field("tnMetodoPago") long metodoPago,
         @Field("tcMonto") String monto,
...
public function getmetodosbyToken($tnTokenService) {
   ...
   return $resultado;
    ...
    public function MandarAyudaQr($tnTransaccion )
    {
     ...
     $url= 'http://chirabot.whatsapp.pagofacil.com.bo/api/ServicioWhatsapp/MandarAyudaWhatsappQr';
         //   https://chirabot.whatsapp.pagofacil.com.bo/api/ServicioWhatsapp/MandarAyudaWhatsappQr
    ...
    public function get_listar_facturashub($lnEmpresa,$lnCodigoFijo,$lnCliente ,$IdOperativo , $FechaOperativa , $NroOperacion , $Servicio , $tnMetodoPago )
    {
      ...
      // $url = 'http://localhost:8000/api/HubPago/getfacturapendiente';
       ...
       $data = array('tnEmpresa' => $lnEmpresa , 'tcCodigoClienteEmpresa' => $lnCodigoFijo ,'tnCliente' => $lnCliente , 'tnIdAccion'=> 17 , 'tcApp'=>2 , "tcIdOperativo"=> $IdOperativo , "tcFechaOperativa"=> $FechaOperativa , 
"tnNroOperacion"=> $NroOperacion , "tnServicio"=> $Servicio , "tnMetodoPago" =>$tnMetodoPago );
   ...
   {
      $url = 'http://serviciopagofacil.syscoop.com.bo/api/Transaccion/getUltimaTransaccionCliente';
      $data = array('tnCliente'=> $idcliente, 'tcApp'=>2 );
   ...
   {
      $url = 'http://serviciopagofacil.syscoop.com.bo/api/Empresa/accesodirectos';
      $data = array('tnCliente'=> $tnCliente, 'tcApp'=>2 );
      /* @POST(cPagoFacilPHP + "/Empresa/listarEmpresas")
    @FormUrlEncoded
    Call<mPaquetePagoFacil<ArrayList<mEmpresaSimple>>> listarEmpresas(
   ...
   public function CambiarContraseñaWhatsapp( $correo , $telefono )
   {
      $url = 'https://serviciopagofacil.syscoop.com.bo/api/Usuario/RecoverPasswordNuevo';
      $data = array('tcCorreo'=> $correo,'tnTelefono'=>$telefono);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
         );
   ...
   public function obtenerpiningreso( $correo , $telefono )
   {
      $url = 'https://serviciopagofacil.syscoop.com.bo/api/Usuario/EnviarPinIngresar';
      $data = array('tcCorreo'=> $correo,'tnTelefono'=>$telefono);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
   ...
   public function getfacturaempresaenvio($tnCliente , $tnEmpresa,$tnMetodoPago)
   {
      $url = 'http://serviciopagofacil.syscoop.com.bo/api/Empresa/getempresaenvio';
      $data = array('tnCliente'=> $tnCliente, 'tnEmpresa'=> $tnEmpresa, 'tnMetodoPago'=> $tnMetodoPago , 'tcApp'=>2 );
      /* @POST(cPagoFacilPHP + "/Empresa/listarEmpresas")
         @FormUrlEncoded
         Call<mPaquetePagoFacil<ArrayList<mEmpresaSimple>>> listarEmpresas(
                  @Field("tnCliente") long cliente,
