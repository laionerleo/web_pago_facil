...
class servicios {
   ...
   public function  prepararpago($tncliente,$tnempresa,$codigoclienteempresa, $tnmetodopago,$tnTelefono , $tcFacturaA , $tnCiNit ,$tcNroPago ,$tnMontoClienteEmpresa , $tnMontoClienteSyscoop , $tcPeriodo , $tcImei , $tcExtension , $tcComplement  , $tcServiceCode , $tcExpireDate , $taFacturas  )
   {
      
     // $url = 'http://serviciopagofacil.syscoop.com.bo/api/Factura/BCP_PrepararPago';
      $url ='http://localhost:8000/api/Factura/BCP_PrepararPago';
...
@Field("tnEmpresa")                 long    empresa,
    ...
    public function EmpresasDetalle($tnEmpresa,$tnCliente)
    {
       ...
       //$url ='http://localhost:8000/api/Empresa/listarEmpresasDetalle';
...
public function realizarpagoatc($datos)
{
   ...
   $url ='http://localhost:8000/api/pay/cybersource_RealizarPago';
...
public function getmetodosbyToken($tnTokenService)
{
   ...
   public function EditarVisitaAgente($lnPuntoCobranza, $lnCliente, $lnSerial, $lcAgente, $lnTelefono)
   {
      $url = 'http://localhost:8000/api/EditarVisitaAgente';
      $data = array('lnPuntoCobranza'=>$lnPuntoCobranza,'lcIdAgente'=>$lnCliente, 'lnSerial'=>$lnSerial, 'lcAgente'=> $lcAgente, 'lnTelefonoAgente'=> $lnTelefono);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
         );
   ...
   public function EditarVisitaPersonalAtendio($lnPuntoCobranza, $lnSerial, $lcAtendio, $lnTelefono)
   {    
      $url = 'http://localhost:8000/api/EditarVisitaAtendio';
      $data = array('lnPuntoCobranza'=>$lnPuntoCobranza, 'lnSerial'=>$lnSerial, 'tcAtendio'=> $lcAtendio, 'tnTelefono'=> $lnTelefono);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
         );
   ...
   public function EditarVisitaPuntoCobranza($lnPuntoCobranza, $dtFecha, $lcDireccion, $lcDescripcion, $lnBanner, $lnSerPunto)
   {
      $url = 'http://localhost:8000/api/EditarVisitaPuntoCobranza';
      $data = array('lnPuntoCobranza'=>$lnPuntoCobranza,'dtFecha'=>$dtFecha, 'lcDireccion'=>$lcDireccion, 'lcDescripcion'=> $lcDescripcion, 'lnBanner'=> $lnBanner, 'lnSerPunto'=>$lnSerPunto);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
         );
   ...
   public function BusarVisitaFechas($dtDesde, $dtHasta)
   {
      $url = 'http://localhost:8000/api/BuscarFechas';
      $data = array('dtDesde'=> $dtDesde, 'dtHasta'=>$dtHasta);
      $header = array(
         "Content-Type: application/x-www-form-urlencoded",
         "Content-Length: ".strlen( http_build_query($data))
         );
